...
function showJMessage() {
  ...
  if (S.responseListeners.length > 0) {
    document
      ...
      .removeEventListener("click", clickOption, true);
    document
      .getElementById("option1")
      .removeEventListener("click", clickOption, true);
    document
  ...
  if (i < message.length) {
    ...
    if (punc == "." || punc == "?" || punc == "!") {
      ...
      if (nextCharIsPunc) {
        if (message[i] == ".") {
          ...
          //for just multiple question marks or exclamation points
      ...
      //tiny pause at a comma
      S.waiting = setTimeout(typeText, 300, message, i);
    } else {
      //no pause for normal text
      S.waiting = setTimeout(typeText, 30, message, i);
    }
  } else {
    ...
    if (window.story.passage(S.currPassage).tags.includes("noexit")) {
      ...
      if (window.story.passage(S.currPassage).tags.includes("toLeave")) {
        ...
        window.story.passage(S.currPassage).tags.includes("toComeback")
      ) {
        S.currPassage = "Come Back";
      } else if (
        window.story.passage(S.currPassage).tags.includes("toWelcome")
      ) {
...
function fadeInResponse(j) {
  ...
  if (j < 1) {
    ...
    for (var i = 0; i < options.length; i++) {
      ...
      S.responseListeners[i] = options[i].addEventListener(
        "click",
        clickOption.bind(this, i),
        true,
      );
    }
  }
